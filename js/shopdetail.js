...
const productId = urlParams.get('idloai');

const products = array.filter(item => item.idLoai === parseInt(productId));


var a = document.getElementById('nameLoai');
if (productId == 100) {
  a.textContent = "Gấu bông teddy";
}

else if(productId==2){
  a.textContent = "Gấu bông hoạt hình"
}
...
message.addEventListener('input', function () {
  ...
  let filteredProducts = products.filter(product => product.ten.toLowerCase().includes(this.value));
...
const amountOutput = document.getElementById('amount');
  ...
  rangeInput.addEventListener('input', function() {
      ...
      let arrayValue = products.filter(product => product.gia >=value);
      loadData(arrayValue);
...
radioButtons.forEach(radioButton => {
   radioButton.addEventListener('change', function() {
       ...
       if (this.checked) {
           ...
           let arrayValue = products.filter(product => product.idLoai == selectedValue);
           //Nếu giá trị tìm >0
...
function addToCart(item) {
  ...
  let value = products.find(product => product.id === item);
  value.quantity = 1;
  let local_storage = [];
